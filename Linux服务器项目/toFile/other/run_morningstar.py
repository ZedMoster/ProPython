...
def to_excel(df_3=None,df_6=None):
    ...
    filename = 'fund_晨星/xml_morningstar.xlsx'
...
if __name__ == '__main__':
    # mongodb服务的地址和端口号
    myClient = pymongo.MongoClient('mongodb://long:ailong.123@localhost:27017')
    # 连接到 数据库
    ...
    d3 = df[(df['base'] == 3.0) & (df['total'] >= 90) &(df['isBuy'] == '开放')].sort_values('total')
    d6 = df[(df['base'] == 6.0) & (df['total'] >= 30) &(df['isBuy'] == '开放')].sort_values('total')
