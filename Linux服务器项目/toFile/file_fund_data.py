...
class ttjj_group_to_excel:
    ...
    def to_fund_rate(self):
        ...
        # 设置过滤条件
        df_1 = data_1[
            key1_0 & key1_1 & key1_2 & key1_3 & key1_4 & key1_5 & key1_6 & key1_7 & key1_8]
    ...
    def group_1(self, df_0, df_1, df_2, df_3, df_5, df_6):
        ...
        # 基金代码
        dx_1 = pd.merge(df_3, df_0,
                        on=['基金代码', '基金名称', '手续费', '单位净值', '今日日期', '是否可购'])
    ...
    def group_5(self, df_0, df_1, df_2, df_3, df_5, df_6):
        ...
        # 基金代码
        dx_5 = pd.merge(df_5, df_2,
                        on=['基金代码', '基金经理', '基金公司_id', '基金经理_id', '基金名称'])
    ...
    def to_group(self, dx_1, dx_2, dx_3, dx_4, dx_5, dx_6):
        _dx = pd.concat([dx_1, dx_2, dx_3, dx_4, dx_5, dx_6], axis=0)
        
        # # 去重
        # _dxEveryone = _dx.drop_duplicates()
        
        # 汇总
        dxGroup = _dx.groupby(['基金代码', '基金名称'],
                              as_index=False).size().sort_values(
            ascending=False).reset_index(name='出现次数')
    ...
    def to_excel_inter(self, df_0, df_1, df_2, df_3, df_5, df_6):
        ...
        with pd.ExcelWriter(filename1) as writer1:
            ...
            # 保存数据
            df_0.to_excel(writer1, sheet_name=name0, index=False,
                          columns=columns_0)
            df_1.to_excel(writer1, sheet_name=name1, index=False,
                          columns=columns_1)
            df_2.to_excel(writer1, sheet_name=name2, index=False,
                          columns=columns_2)
            df_3.to_excel(writer1, sheet_name=name3, index=False,
                          columns=columns_3)
            df_5.to_excel(writer1, sheet_name=name5, index=False,
                          columns=columns_5)
            df_6.to_excel(writer1, sheet_name=name6, index=False,
                          columns=columns_6)
    ...
    def main(self):
        try:
            ...
            dxGroup = self.to_group(dx_1, dx_2, dx_3, dx_4, dx_5, dx_6)
            # 次数大于 1
            _dxGroup = dxGroup[dxGroup['出现次数'] > 1]
            
            self.to_excel_inter(df_0, df_1, df_2, df_3, df_5, df_6)
            self.to_excel_outer(_dxGroup)
...
if __name__ == '__main__':
    ...
    try:
        ...
        print("{} -- good job to files".format(strTime))
    
    except Exception as identifier:
        print('{} ** no job to files'.format(strTime) + identifier)
